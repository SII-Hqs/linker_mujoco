"""Runtime target q/qdot ESN torque controller for MuJoCo experiments.

Typical use:

    controller = ESNTorqueController(
        "results/esn_controller_target_q_dq_7_seed30_60hz.npz",
        torque_limit=[2.0, 2.0, 2.0],
    )

    # Call at 60 Hz, matching the training data.
    tau = controller.step(qpos3, qvel3, force_g, target_qpos3, target_qvel3, horizon=1)
...
class ESNTorqueController:
    """Online ESN controller exported by save_esn_controller_target_q_dq_7.py."""
    ...
    def __init__(
        ...
        self.target_state_offset = int(model["target_state_offset"])
        ...
        self.target_qvel_filter = OnePoleLowpass(cutoff_hz=cutoff, sample_hz=self.fs_hz)
        ...
        if self.input_dimension != 13:
            raise ValueError(f"Expected input_dimension=13, got {self.input_dimension}")
    ...
    def build_input(
        ...
        target_qvel: Iterable[float],
    ) -> np.ndarray:
        """Build and standardize the 13-D ESN input vector."""
        ...
        target_qvel = np.asarray(target_qvel, dtype=float).reshape(-1)
        if qpos.shape != (3,):
        ...
        if target_qpos.shape != (3,):
            raise ValueError("target_qpos must contain exactly 3 joint positions")
        if target_qvel.shape != (3,):
            raise ValueError("target_qvel must contain exactly 3 joint velocities")

        qvel_filtered = self.qvel_filter.step(qvel)
        force_filtered = float(self.force_filter.step(np.asarray(force_g, dtype=float)))
        target_qvel_filtered = self.target_qvel_filter.step(target_qvel)
        raw = np.concatenate([
            qpos,
            qvel_filtered,
            [force_filtered],
            target_qpos,
            target_qvel_filtered,
        ])
    ...
    def predict_all_horizons(
        ...
        """Advance the reservoir by one control tick and return all 9 outputs."""
        u = self.build_input(
            qpos,
            qvel,
            force_g,
            target_qpos,
            target_qvel,
        ).reshape(self.input_dimension, 1)
    ...
    def step(
        ...
        y = self.predict_all_horizons(qpos, qvel, force_g, target_qpos, target_qvel)
    ...
    def select_horizon(self, y: np.ndarray, horizon: int) -> np.ndarray:
        """Select joint torques for one horizon from the 9-D ESN output."""
...
class ControlRateAdapter:
    ...
    def step(
        ...
        if self.elapsed >= self.update_period:
            ...
            self.last_tau = self.controller.step(
                qpos,
                qvel,
                force_g,
                target_qpos,
