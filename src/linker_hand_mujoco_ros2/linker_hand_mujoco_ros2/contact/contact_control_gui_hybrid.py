#!/usr/bin/env python3
"""Interactive PyQt5 GUI for the L20a hybrid-control contact demo."""
...
if __package__:
    from .contact_control_demo_hybrid import (
        add_hybrid_arguments,
    ...
    from linker_hand_mujoco_ros2.contact.contact_control_demo_hybrid import (
        add_hybrid_arguments,
        build_hybrid_simulation,
    )
...
class HybridContactControlWindow(QMainWindow):
    """GUI for XYZ platform control and hybrid force-target tuning."""
    ...
    def __init__(self, worker, position_range, refresh_ms):
        ...
        self.setWindowTitle("L20a Hybrid Contact Platform Control")
    ...
    def _build_content(self):
        ...
        force_group = QGroupBox("Hybrid Force Target")
        ...
        self.pos_delta_label = QLabel("0.00000 rad")
        self.pos_delta_label.setAlignment(Qt.AlignRight | Qt.AlignVCenter)
        ...
        contact_layout.addRow("Position delta:", self.pos_delta_label)
    ...
    def refresh_status(self):
        ...
        controller = self.worker.simulation.controller
        ...
        self.pos_delta_label.setText(f"{controller.last_position_delta:.5f} rad")
...
def parse_args(argv):
    """Parse hybrid GUI and shared simulation arguments."""
    parser = argparse.ArgumentParser(
        description="Interactively control the L20a hybrid contact platform."
    )
...
def main():
    """Launch the hybrid-control GUI and simulation worker."""
    args = parse_args(sys.argv[1:])
    simulation = build_hybrid_simulation(args)
    worker = SimulationWorker(simulation, launch_viewer=not args.no_viewer)

    app = QApplication(sys.argv[:1])
    window = HybridContactControlWindow(worker, args.position_range, args.refresh_ms)
    worker.start()
    window.show()
    exit_code = app.exec_()
    worker.stop()
